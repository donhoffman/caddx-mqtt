...
class MQTTClient(object):
    def __init__(self, host, port, username, password, state_topic_root, command_topic, tls_active, tls_insecure,
                 timeout_sec):
    ...
    def publish_partition_condition_flags(self, partition, flagList=None, qos=0, retain=True):
    ...
    def publish_zone_type_flags(self, zone, flagList=None, qos=0, retain=True):
        topic = self._state_topic_root + "/zones/" + str(zone) + "/types"
        flags = str(flagList)
        self.publish(topic, flags, qos, retain)

    def publish_zone_condition_flags(self, zone, flagList=None, qos=0, retain=True):
        topic = self._state_topic_root + "/zones/" + str(zone) + "/conditions"
        flags = str(flagList)
        self.publish(topic, flags, qos, retain)

    def publish_system_datetime(self, _qos=0, _retain=True):
        now = datetime.datetime.now()
        self.publish(self._state_topic_root + "/system/datetime", str(now.strftime("%m/%d/%Y, %H:%M:%S")), retain=False)
